...
## Dependency audit — baseline 2026-09-10

This is the last **recorded** audit, not a live figure. Re-run `npm audit` in `backend/` and
`frontend/` at every CP-SEC and update this section.

Toolchain: Node 22 images, Express 5, Prisma, Vite 7, React 19.

| Scope | State at the baseline |
|-------|-----------------------|
| Backend | **One moderate, accepted.** `adm-zip` 0.6.0 (the newest release) — extraction follows symlinks in the destination. No fixed version exists; the advised "fix" is a *downgrade* to 0.5.8, which does not address it. ReView never reaches that code path with an untrusted entry: `lib/zipSafety.ts` decides from the archive's central directory, before a single byte is written, and refuses any symlink entry outright — all-or-nothing. Earlier fix kept: `adm-zip` ≥ 0.6.0 for the crafted-ZIP memory exhaustion (high). |
...
The agent that lets the administration back up and switch releases holds `/var/run/docker.sock`.
On that machine, that is equivalent to root: a container that talks to the daemon can start
another one with the host disk mounted. It is therefore optional, installed deliberately, and
built around one question — *what does a compromise of the application actually buy?*

| Property | Why |
|----------|-----|
| The socket is on the agent alone | The backend is reachable from the internet, runs as root and processes user files. It never receives the socket, and a test asserts no compose file of the stack mounts one. |
| Two separate mounts | The backend **writes** `ops/queue` and reads `ops/state` **read-only**; the agent does the opposite. Sharing one writable directory would let a compromised backend plant a symlink the agent — that is, the daemon — would follow when writing. |
| Three orders, no free-form command | `update`, `backup`, `verify`. Every field is filtered three times: when the order is built, by `update.sh`, and by the agent. A duration ends up inside a bash arithmetic expression, which re-evaluates the contents of the variables it meets. |
| `restore.sh db|all` is never orderable | Restoring overwrites without asking. Only the non-destructive `verify` can be commanded. |
| Permissions live in `deploy/agent.conf` | That file is mounted in **no** container of the application. Someone holding a stolen admin session cannot grant themselves what the operator refused, and cannot downgrade the instance to a vulnerable release. |
| The password step-up is not the barrier | Asking for the admin password stops the accidental click and the tab left open. It is not a defence against a stolen session — whoever holds one can also create an account — and this page does not pretend otherwise. `agent.conf` is the barrier. |

The agent runs with no network of its own (`network_mode: "none"`): `pull`, `run` and `exec`
are carried out by the daemon, not by the container. Its output is redacted before it reaches a
browser — known secret values, API tokens and credential URLs are replaced by string substitution,
never by a regular expression built from a password.
