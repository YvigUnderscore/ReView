...
| Side | Strategy | Cost |
|------|----------|------|
| Frontend | **No catalogue is bundled, English included.** All fourteen are lazy chunks resolved by `import.meta.glob('./messages/*.json')`; `initLocale()` fetches the chosen language, and English alongside it only when English will actually be read | 161–283 kB raw per language, **51–64 kB gzipped**; one round trip, because the two requests go out in parallel |
...
### Why English is not bundled either

It used to be: `import base from './messages/en.json'` inlined every key into the entry chunk.
Measured on a real build, that literal was **some 50 kB gzipped of the first load**, and a
reader in Paris paid for it before downloading the French catalogue that would answer every one
of those keys. English is now a chunk like the other thirteen, and the entry chunk holds no
message at all — `MessageKey` is derived through `typeof import('./messages/en.json')`, a
*type*, which TypeScript resolves at compile time and leaves nothing of in the bundle.

English is still the fallback, key by key, so it has to be there **when it is needed** and only
then. `vite.config.js` reads the catalogues it is about to ship and lists the languages that
cover every English key in `import.meta.env.I18N_FULL_LOCALES`; `coversAllBaseKeys()` consults
that list at runtime:

| Chosen language | What is fetched |
|---|---|
| Covers every English key | That catalogue alone — English would never be read |
| Leaves keys uncovered | That catalogue **and** English, both requested in parallel |
| List unavailable (dev server, tests, a build without that `define`) | That catalogue and English — when in doubt, the fallback wins over the saving |

All thirteen translations cover the English key set today, so nobody but an English reader
downloads `en`. Let a partial catalogue land and its language simply leaves the list, English
travels with it, and nothing on screen changes.

Outside a production build the English catalogue *is* embedded eagerly, because unit tests call
`t()` without ever running `initLocale()` and the dev server has no chunk to save. That branch
sits behind `import.meta.env.PROD`, which Vite resolves at build time — it disappears,
catalogue included, from what is shipped.

> [!IMPORTANT]
> Adding a language costs nothing to readers who do not pick it, but it is not free for the one
> who does: a catalogue is 51 to 64 kB over the wire. Dead keys therefore cost fourteen times
> over — see [the fifth check](#the-five-checks), which counts them.
...
- **Name label props after the known list** — `label`, `title`, `hint`, `placeholder`, `alt`,
  `description`, `message`, `aria-label`, `subtitle`, `tooltip`, `confirmLabel`, `keys`… A
  component that invents a short prop name (`<Row k= v=>`) hides its text from the checker.
  Rename the prop rather than weaken the check. A list handed to one of those props is read
  element by element, so `keys={['Space', 'Shift']}` is two strings to translate, not one
  opaque expression.
...
2. Otherwise the studio default, set in **Admin → Studio → Studio identity → Studio language**
...
TypeScript AST of **`frontend/src` and `backend/src`** and reports **every literal that
reaches the user**: JSX text, visible props, arguments of `toast`/`confirm`/`alert`,
template fragments around an interpolation, fallback values such as `{name ?? 'Anonymous'}`.
Identifiers, CSS classes, URLs, GLSL, units, acronyms and glossary terms are excluded
explicitly, in `SKIP` and `ALLOWED` — not guessed.

Two widenings were paid for by real regressions, and both are worth knowing when you name a
prop:

- **`keys` is a visible prop.** The shortcut help writes a chord as `keys={['Space',
  'Shift']}`, and the prop escaped the check for the single reason that it was not called
  `label`. Key names stayed French in the thirteen other catalogues for the whole migration.
- **An array reaching a visible prop is read element by element.** The walk used to stop at
  the opening bracket. Each element is now reported **separately** rather than joined:
  a chord is not a sentence, and gluing `['Ctrl', 'K']` together would make two names nobody
  translates look like interface text.

Two ceilings, one per tree, and each may only go down:

| Tree | Ceiling | What it means |
|------|---------|---------------|
| `frontend/src` | **0** | No literal reaches the interface. This is the rule, not a debt |
| `backend/src` | **330** | A baseline, frozen the day the backend entered the scope. Almost all of it is outside any interface — SVG assembly, FFmpeg filters, SQL fragments, operator log lines, ShotGrid error text. The surfaces a human actually reads are at zero and go through `t()`: the public `/api/docs` footer, the unsubscribe page, the e-mails |

The backend figure is a debt written down, not a licence: it went 343 → 332 when tagged
`Prisma.sql` templates stopped being counted, and 332 → 330 when the video trim service and
its router were deleted with the feature. It is declared as `BACKEND_DEBT` in the script.
