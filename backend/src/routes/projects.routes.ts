...
router.post(
  ...
  async (req, res) => {
    const { name, description, startFrame } = req.body as { name: string; description?: string; startFrame?: number };
    ...
    logAudit({ userId: req.user!.id, action: 'PROJECT_CREATE', entityType: 'Project', entityId: project.id, metadata: { name } });
...
router.patch(
  ...
  async (req, res) => {
    ...
    if (!(await prisma.project.findFirst({ where: { id, deletedAt: null } }))) throw notFound('Projet introuvable');
...
router.get(
  ...
  async (req, res) => {
    ...
    const [sequences, shots, assets, versions, media] = await Promise.all([
      prisma.sequence.findMany({ where: { projectId, deletedAt: { not: null } }, orderBy: { deletedAt: 'desc' } }),
...
router.put(
  ...
  async (req, res) => {
    ...
    logAudit({ userId: req.user!.id, action: 'PROJECT_SETTINGS_UPDATE', entityType: 'Project', entityId: id });
    res.json({ settings });
  },
);
...
router.get(
  ...
  async (req, res) => {
    ...
    const [versions, media, tasks] = await Promise.all([
      prisma.version.findMany({
        ...
        include: {
          ...
          task: { select: { id: true, name: true, shot: { select: { code: true, sequence: { select: { code: true } } } }, asset: { select: { name: true } } } },
      ...
      prisma.mediaObject.findMany({
        ...
        include: {
          ...
          version: { select: { name: true, task: { select: { id: true, name: true, shot: { select: { code: true, sequence: { select: { code: true } } } }, asset: { select: { name: true } } } }, asset: { select: { name: true } } } },
        },
      }),
    ...
    const loc = (t: { shot?: { code: string; sequence?: { code: string } | null } | null; asset?: { name: string } | null } | null) => {
      if (!t) return '';
    ...
    const recent = [
      ...versions.map((v) => ({
        type: 'version' as const, id: v.id, at: v.createdAt,
        label: `${v.name}${v.task ? ' — ' + v.task.name : ''}`,
        location: loc(v.task), versionId: v.id, taskId: v.task?.id ?? null,
        author: v.author?.name ?? null,
      })),
      ...media.map((m) => ({
        type: 'media' as const, id: m.id, at: m.createdAt,
        label: m.originalName, kind: m.kind,
        location: loc(m.version?.task ?? null) || (m.version?.asset?.name ?? ''),
        mediaId: m.id, taskId: m.version?.task?.id ?? null,
...
router.delete(
  ...
  async (req, res) => {
    await prisma.projectMembership.delete({
      where: { userId_projectId: { userId: Number(req.params.userId), projectId: Number(req.params.projectId) } },
    });
    res.status(204).end();
  },
);
