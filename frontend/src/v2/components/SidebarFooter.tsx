  ...
  return (
    <div className="border-t border-border">
      ...
      <div className="px-3 py-2">
        ...
        >
          <span className="flex items-center gap-1.5"><Users size={14} /> En ligne · {onlineCount}</span>
          <ChevronDown size={14} className={presenceOpen ? 'rotate-180 transition-transform' : 'transition-transform'} />
        </button>
        {presenceOpen && (
          <div className="custom-scrollbar mt-1 max-h-44 space-y-0.5 overflow-y-auto">
            {others.length === 0 && <p className="px-2 py-1 text-xs text-muted-foreground">Personne d’autre.</p>}
            {others.map((u) => (
              <div key={u.id} className="flex items-center gap-2 rounded-md px-2 py-1" title={STATUS_LABEL[u.status]}>
                <Avatar seed={u.id} initials={u.initials} avatarUrl={u.avatarUrl} size={24} status={u.status} online={u.online} />
                <div className="min-w-0 flex-1">
                  <div className="truncate text-xs font-medium">{u.displayName}</div>
                  <div className="truncate text-[10px] text-muted-foreground">{lastSeenLabel(u.lastSeenAt, u.online)}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="relative border-t border-border p-3">
        <div className="mb-2 flex items-center gap-2">
          <Avatar seed={user.id} initials={user.initials ?? user.email.slice(0, 2).toUpperCase()} avatarUrl={user.avatarUrl} size={32} status={myStatus} online />
          <div className="min-w-0 flex-1">
            <div className="truncate text-sm font-medium">{user.displayName ?? user.name ?? user.email}</div>
            <button onClick={() => setStatusOpen((o) => !o)} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground">
              <span className="inline-block h-2 w-2 rounded-full" style={{ background: STATUS_COLOR[myStatus] }} />
        ...
        {statusOpen && (
          <div className="absolute bottom-16 left-3 z-20 w-44 rounded-md border border-border bg-card p-1 shadow-lg">
            {STATUSES.map((s) => (
              <button key={s} onClick={() => setStatus(s)} className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-xs hover:bg-secondary">
