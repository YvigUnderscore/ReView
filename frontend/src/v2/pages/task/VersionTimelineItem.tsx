...
function MediaTile({ media, canManage, onPublish, onDelete }: {
  media: MediaSummary; canManage: boolean;
  onPublish: (m: MediaSummary) => void; onDelete: (m: MediaSummary) => void;
}) {
    ...
    <div className="group overflow-hidden rounded-md border border-border bg-card">
      <Link to={`/review/${media.id}`} title={`Ouvrir la review : ${media.originalName}`} className="block">
        <div className="relative flex aspect-video items-center justify-center bg-black/40 text-muted-foreground">
          <Icon size={22} />
          {!media.published && (
            <span className="absolute left-1 top-1 rounded bg-primary/20 px-1 text-[10px] text-primary">Brouillon</span>
          )}
        ...
        <div className="truncate px-1.5 pb-1 text-[10px] text-muted-foreground">{media.kind} · {media.status}</div>
      </Link>
      {canManage && (
        <div className="flex border-t border-border text-[10px]">
          {!media.published && (
            <button onClick={() => onPublish(media)} className="flex-1 py-1 text-muted-foreground hover:bg-secondary hover:text-foreground">Publier</button>
          )}
          <button onClick={() => onDelete(media)} title="Supprimer" className="flex items-center justify-center px-2 py-1 text-muted-foreground hover:bg-secondary hover:text-destructive">
            <Trash2 size={12} />
          </button>
        </div>
      )}
...
export default function VersionTimelineItem({
  version, isLast, defaultOpen, canCreate, canPublish,
  onUpload, onPublishVersion, onDeleteVersion, onPublishMedia, onDeleteMedia,
}: {
  version: VersionListItem; isLast: boolean; defaultOpen: boolean;
  canCreate: boolean; canPublish: boolean;
  ...
  return (
    <li className="relative flex gap-3 pb-4">
      <div className="relative flex flex-col items-center pt-1.5">
        <span className={`z-10 h-3 w-3 shrink-0 rounded-full ring-2 ring-background ${VERSION_STATUS_DOT[version.status]}`} />
      ...
      <div className="min-w-0 flex-1 rounded-lg border border-border bg-card">
        <div className="flex flex-wrap items-center justify-between gap-2 px-3 py-2">
          <button onClick={() => setOpen((o) => !o)} className="flex min-w-0 items-center gap-2 text-left">
            ...
            <span className={`rounded px-1.5 py-0.5 text-[10px] ${VERSION_STATUS_COLOR[version.status]}`}>{VERSION_STATUS_LABEL[version.status]}</span>
            <span className="truncate text-xs text-muted-foreground">
              {version.author?.name ? `${version.author.name} · ` : ''}{timeAgo(version.createdAt)} · {version._count.media} média{version._count.media > 1 ? 's' : ''}
            </span>
          </button>
          <div className="flex shrink-0 items-center gap-1.5">
            {canCreate && (
              <Button size="sm" variant="outline" onClick={() => onUpload(version.id)}><Upload size={13} /> Média</Button>
            )}
            {canPublish && !version.published && (
              <Button size="sm" variant="outline" onClick={() => onPublishVersion(version.id)}>Publier</Button>
            )}
            {canCreate && (
              <Button size="sm" variant="ghost" onClick={() => onDeleteVersion(version)} className="text-muted-foreground hover:text-destructive">
                <Trash2 size={13} />
              </Button>
            )}
          </div>
        ...
        {open && (
          <div className="border-t border-border p-3">
            {mediaQ.isLoading ? (
              ...
              <p className="text-xs text-muted-foreground">Aucun média dans cette version.{canCreate ? ' Utilisez « Média » ou déposez un fichier ci-dessus.' : ''}</p>
            ) : (
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
                {media.map((m) => (
                  <MediaTile
