...
export const VERSION_STATUS_LABEL: Record<VersionStatus, string> = {
  DRAFT: 'Brouillon', REVIEW: 'En review', PUBLISHED: 'Publiée',
};
...
export const VERSION_STATUS_DOT: Record<VersionStatus, string> = {
  DRAFT: 'bg-muted-foreground/50', REVIEW: 'bg-amber-400', PUBLISHED: 'bg-green-400',
};

/** Icône représentant le type de média (vignette de repli). */
export const MEDIA_KIND_ICON: Record<MediaKind, ComponentType<{ size?: number | string; className?: string }>> = {
  VIDEO: FileVideo, IMAGE: Image, MODEL_3D: Box,
};
