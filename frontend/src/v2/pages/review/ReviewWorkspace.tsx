...
import { resolveGlbSrc, type MediaResp } from './reviewTypes';
import { usePublishSceneOverride } from './usePublishSceneOverride';
import { useAnnotations } from './useAnnotations';
import { useCommentSelection } from './useCommentSelection';
import { useMontageWiring } from './useMontageWiring';
import { loadDraft, saveDraft } from './commentDraft';
...
import { useSplatThumbnail } from './useSplatThumbnail';
import { useAutoThumbnail } from './useAutoThumbnail';
import { useModel3DThree } from './three/useModel3DThree';
import ReviewHeader from './ReviewHeader';
import ReviewViewer from './ReviewViewer';
import MontageTrack, { type MontageContext } from './MontageTrack';
import { useSplatPaint } from './splat/paint/useSplatPaint';
import { useSplat } from './splat/useSplat';
import CommentsPanel from './CommentsPanel';
import TheaterExitButton from './TheaterExitButton';

/**
 * Espace de review d'un média (vidéo/image/3D) — orchestrateur des panes (découpage 10.C2).
 *
 * Deux écrans s'en servent : la review d'un média (`/review/:mediaId`) et le montage
 * (`/timelines/:id/play`), qui n'est rien d'autre que cette review appliquée au plan
 * courant, avec la bande du film sous le viewer. Les outils sont donc les mêmes partout :
 * un montage se review avec l'annotation, la boucle, l'image par image et la comparaison
 * A/B, pas avec un sous-ensemble réinventé pour lui.
 */
export default function ReviewWorkspace({
  ...
  id: number;
  rawParam?: string;
  /** Titre de la coquille — le nom du montage plutôt que celui du fichier, le cas échéant. */
  title?: string;
  /** Contexte de montage : bande du film, entrée dans le plan, enchaînement. */
  ...
  // Commentaire affiché : sa sélection restaure ensemble frame, annotation et caméra.
  const {
    selectedId: selectedCommentId,
    select: selectComment,
    clear: clearSelection,
    seek,
  } = useCommentSelection({ data, ann, paint, splat, model3d, videoRef, programmaticSeekRef });
  ...
  // Page de montage (46) : entrée dans le plan à la bonne position, puis enchaînement.
  useMontageWiring({ montage, videoRef, programmaticSeekRef, ready: !!data });
  ...
  return (
    <Shell
      title={title ?? data?.media.originalName ?? 'Review'}
        ...
        <div className="flex min-h-0 flex-1 gap-4">
          {montage ? (
            <div className="flex min-h-0 min-w-0 flex-1 flex-col gap-2">
              {viewer}
              {/* La bande du film sous le lecteur : le montage entier reste visible, et
                  cliquer dedans emmène le plan voulu dans cette même review. */}
              <MontageTrack montage={montage} videoRef={videoRef} videoReady={!!data} />
            </div>
          ) : (
            viewer
          )}
