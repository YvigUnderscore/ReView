...
export default function Model3DToolbar({ m, showEditTools }: { m: Model3DState; showEditTools: boolean }) {
  return (
    <div className="flex shrink-0 flex-wrap items-center gap-2 rounded-md border border-border bg-card px-3 py-2 text-xs">
      ...
      {m.freeCamera && <span className="text-[11px] text-muted-foreground">Translation : clic droit / Maj+glisser / 2 doigts</span>}
        ...
        <>
          ...
          {m.animations.length > 1 ? (
            <label className="flex items-center gap-1.5 rounded-md bg-secondary/40 px-2 py-1 text-muted-foreground">
              <Film size={13} />
              <select value={m.currentAnim ?? ''} onChange={(e) => m.selectAnim(e.target.value)} className="rounded border border-input bg-background px-1 py-0.5 text-xs text-foreground">
                {m.animations.map((a) => <option key={a} value={a}>{a}</option>)}
              </select>
            </label>
          ) : (
        ...
        <>
          ...
          {(['yaw', 'pitch', 'roll'] as const).map((axis) => (
            <label key={axis} className="flex items-center gap-1.5 rounded-md bg-secondary/40 px-2 py-1 text-muted-foreground">
              ...
              <input type="range" min={-180} max={180} step={1} value={m.transform[axis]}
                onChange={(e) => m.updateTransform({ [axis]: Number(e.target.value) })} className="w-24" />
          ...
          <label className="flex items-center gap-1.5 rounded-md bg-secondary/40 px-2 py-1 text-muted-foreground">
            ...
            <input type="range" min={0.1} max={5} step={0.1} value={m.transform.scale}
              onChange={(e) => m.updateTransform({ scale: Number(e.target.value) })} className="w-24" />
            <span className="w-8 text-right font-mono text-foreground">{m.transform.scale.toFixed(1)}</span>
          </label>
          <button onClick={m.saveTransform} className="flex items-center gap-1.5 rounded-md bg-primary px-2.5 py-1.5 font-medium text-primary-foreground">
            <Save size={13} /> {m.savedTf ? 'Enregistré' : 'Enregistrer'}
          </button>
          <button onClick={() => m.updateTransform(DEFAULT_TRANSFORM)} title="Réinitialiser la transformation" className="flex items-center justify-center rounded-md border border-border p-1.5 hover:bg-secondary/60">
            <RotateCcw size={13} />
          </button>
        </>
      )}
