...
export default function VideoPane({
  src, videoRef, programmaticSeekRef, overlay, comments, selectedId,
  onSelectComment, onManualSeek, onMarker, fps, fpsDetected, setFpsOverride, startFrame,
}: {
  src: string;
  ...
  const seekTo = (t: number) => {
    ...
    if (v) { programmaticSeekRef.current = true; v.currentTime = t; }
  };
  ...
  return (
    <>
      ...
      <div className="flex shrink-0 flex-wrap items-center gap-3 rounded-md border border-border bg-card px-3 py-1.5 text-xs">
        ...
        <button onClick={() => stepVideoFrame(videoRef.current, fps, -1)} title="Frame précédente (←)" className="rounded border border-border px-2 py-0.5 hover:bg-secondary/60">◀ -1</button>
        <button onClick={() => stepVideoFrame(videoRef.current, fps, 1)} title="Frame suivante (→)" className="rounded border border-border px-2 py-0.5 hover:bg-secondary/60">+1 ▶</button>
        <span className="text-muted-foreground">|</span>
        <label className="flex items-center gap-1 text-muted-foreground">
          fps
          <input type="number" value={fps} min={1} max={120} disabled={fpsDetected}
