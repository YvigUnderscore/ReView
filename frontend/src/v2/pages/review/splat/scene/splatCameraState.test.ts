...
const makeControls = (target: THREE.Vector3) => ({ target, update: vi.fn() }) as never;
...
describe('splatCameraState — capture/restauration avec tilt (roll)', () => {
  it('capture position/cible/fov et omet le roll si horizon droit', () => {
    ...
    const cam = captureSplatCamera(THREE, camera, makeControls(new THREE.Vector3(0, 0, 0)));
  ...
  it('restaure position/cible/fov et applique le roll (up incliné)', () => {
    ...
    const controls = makeControls(new THREE.Vector3());
  ...
  it('état inexploitable : no-op', () => {
    ...
    restoreSplatCamera(THREE, camera, makeControls(new THREE.Vector3()), null);
