...
function ModelViewer({ src, innerRef, transform, hotspots, freeCamera, animationName }: {
  src: string; innerRef: RefObject<HTMLElement | null>; transform: Transform;
...
export default function Model3DPane({
  status, ready, glbSrc, modelRef, transform, freeCamera, hotspots, animationName,
  overlay, canReprocess, reprocessing, onReprocess,
}: {
  status: string; ready: boolean; glbSrc: string | null;
  modelRef: RefObject<HTMLElement | null>; transform: Transform; freeCamera: boolean;
  hotspots: Hotspot3D[]; animationName?: string; overlay: ReactNode;
  canReprocess: boolean; reprocessing: boolean; onReprocess: () => void;
}) {
  return (
      ...
      {status === 'PROCESSING' ? (
        ...
        <>
          <ModelViewer
            ...
            hotspots={hotspots} animationName={animationName}
          />
