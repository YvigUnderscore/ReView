...
export default function UsersTab() {
  ...
  const { data: users, isLoading } = useQuery({ queryKey: qk.users, queryFn: () => api.get<{ users: User[] }>('/api/users').then((d) => d.users) });
  ...
  const confirmDelete = async () => {
    if (!deleting) return;
    try { await api.del(`/api/users/${deleting.id}`); toast.success('Utilisateur supprimé'); setDeleting(null); invalidate(); }
  ...
  return (
    <div>
      <div className="mb-2 flex items-center justify-between">
        ...
        <Button size="sm" onClick={() => setCreating(true)}><Plus size={14} /> Nouvel utilisateur</Button>
      </div>
              ...
              <tr key={u.id} className="border-t border-border">
                ...
                <td className="px-3 py-2">
                  <div className="flex justify-end gap-1">
                    <button onClick={() => setEditing(u)} title="Modifier" className="rounded p-1 text-muted-foreground hover:bg-secondary hover:text-foreground"><Pencil size={15} /></button>
                    {u.id !== meId && <button onClick={() => setDeleting(u)} title="Supprimer" className="rounded p-1 text-destructive hover:bg-secondary"><Trash2 size={15} /></button>}
                  </div>
                </td>
              </tr>
            ))}
      ...
      {creating && <UserModal title="Nouvel utilisateur" onClose={() => setCreating(false)} onSaved={() => { setCreating(false); invalidate(); }} />}
      {editing && <UserModal title="Modifier l'utilisateur" user={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); invalidate(); }} />}
      <ConfirmDialog
        open={!!deleting}
