...
export default function ProjectDefaultsTab() {
  ...
  const { data } = useQuery({
    ...
    queryFn: () => api.get<{ settings: ProjectSettings }>('/api/admin/project-defaults').then((d) => d.settings),
  });
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <Panel title="Nomenclature par défaut">
        <div className="flex flex-wrap items-end gap-3">
          <DefField label="Préfixe séquence"><Input className="w-24 py-1.5 text-xs" value={draft.nomenclature.sequencePrefix} onChange={(e) => setNom('sequencePrefix', e.target.value)} /></DefField>
          <DefField label="Préfixe shot"><Input className="w-24 py-1.5 text-xs" value={draft.nomenclature.shotPrefix} onChange={(e) => setNom('shotPrefix', e.target.value)} /></DefField>
          <DefField label="Pas"><Input type="number" min={1} className="w-16 py-1.5 text-xs" value={String(draft.nomenclature.step)} onChange={(e) => setNom('step', e.target.value)} /></DefField>
          <DefField label="Chiffres"><Input type="number" min={1} max={8} className="w-16 py-1.5 text-xs" value={String(draft.nomenclature.padding)} onChange={(e) => setNom('padding', e.target.value)} /></DefField>
        </div>
      </Panel>
        ...
        <div className="space-y-1.5">
          {draft.departments.map((dep, i) => (
            <div key={i} className="flex items-center gap-2">
              <Input className="w-32 py-1.5 text-xs" placeholder="Clé" value={dep.key} onChange={(e) => setDept(i, 'key', e.target.value)} />
              <Input className="flex-1 py-1.5 text-xs" placeholder="Nom" value={dep.name} onChange={(e) => setDept(i, 'name', e.target.value)} />
