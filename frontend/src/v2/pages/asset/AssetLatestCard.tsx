 ...
 *
 * Le lien « permanent » copié ici pointe sur `/assets/:id/latest`, qui redirige toujours
 * vers l'état courant : collé dans une note de production, il reste juste des mois plus
 * tard sans que personne ait à le remettre à jour.
 */
export default function AssetLatestCard({ assetId, latest }: { assetId: number; latest: AssetLatest }) {
  const t = useT();
  const media = latest.media;

  const copyPermalink = async () => {
    const url = `${window.location.origin}/assets/${assetId}/latest`;
    try {
