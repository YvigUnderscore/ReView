...
interface TaskEvent { projectId: number; id: number; shotId: number | null; assetId: number | null }
interface VersionEvent { projectId: number; id: number; taskId: number | null; assetId: number | null }
interface MediaEvent { projectId: number; id: number; versionId: number }

const COMMENT_EVENTS = ['comment:new', 'comment:update', 'comment:delete', 'comment:reaction', 'comment:reaction:remove'];
...
export function useSocketInvalidation(projectId: number | null): void {
  ...
  useEffect(() => {
    ...
    const onComment = () => { qc.invalidateQueries({ queryKey: ['comments'] }); };
    ...
    const onVersion = (e: VersionEvent) => {
      const parent = e.taskId != null ? `taskId=${e.taskId}` : e.assetId != null ? `assetId=${e.assetId}` : null;
